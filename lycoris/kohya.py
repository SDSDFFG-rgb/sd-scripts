...
def parse_nest_size_kwargs(nw_kwargs):
    nest_dims = nw_kwargs.get("nest_dim", None)
    if nest_dims is None:
        return None
    nest_dims = [int(s) for s in nest_dims.split(",")]
    if len(nest_dims) != 2:
        raise ValueError("nest_dim must have two values like '512,704'")
    culc = 8
    for i in range(2):
        culc *= 2
        nest_dims[i] = nest_dims[i] // culc
    logger.info(f"nest setting: {nest_dims}")
    return nest_dims
...
def get_nest_dim_for_lora(lora_name: str, nest_dims):
    if nest_dims is None:
        return None
    ...
    if block_idx < NUM_OF_BLOCKS:
        nest_id = block_idx // 4 - 1
        if nest_id >= 0:
            return nest_dims[nest_id]
    elif block_idx > NUM_OF_BLOCKS:
        nest_id = (block_idx - NUM_OF_BLOCKS - 1) // 4
        if nest_id < 2:
            return nest_dims[1 - nest_id]
    return None
...
def create_network(
    ...
    nest_dims = parse_nest_size_kwargs(kwargs)
        ...
        **kwargs,
        ...
        def create_modules_(
            ...
            for name, module in root_module.named_modules():
                ...
                nest_dim = None
                if is_unet and self.nest_dims is not None:
                    if isinstance(module, (torch.nn.Conv2d, torch.nn.LayerNorm, torch.nn.GroupNorm)):
                        nest_dim = get_nest_dim_for_lora(lora_name, self.nest_dims)
                module_config = configs
                if nest_dim is not None:
                    module_config = {**configs, "nest_dim": nest_dim}
                lora = create_single_module(lora_name, module, algo, **module_config)
                if lora is not None:
                    loras[lora_name] = lora
                    lora_names.append(lora_name)
            ...
            for name, module in root_module.named_modules():
                ...
                if module_name in target_replace_modules and not any(
                    ...
                    if module_name in self.MODULE_ALGO_MAP:
                        ...
                        algo = network_module
                    loras.extend(
                        create_modules_(f"{prefix}_{name}", module, algo, is_unet, next_config)[
                            0
                        ]
                    )
                    next_config = {}
                    ...
                    if is_unet and self.nest_dims is not None:
                        if isinstance(module, (torch.nn.Conv2d, torch.nn.LayerNorm, torch.nn.GroupNorm)):
                            nest_dim = get_nest_dim_for_lora(lora_name, self.nest_dims)
                    module_config = next_config
                    if nest_dim is not None:
                        module_config = {**next_config, "nest_dim": nest_dim}
                    lora = create_single_module(lora_name, module, algo, **module_config)
                    next_config = {}
                    if lora is not None:
                        loras.append(lora)
