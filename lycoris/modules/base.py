...
class LycorisBaseModule(ModuleCustomSD):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if nest_dim is not None:
            self.nest_dim = int(nest_dim) * int(nest_dim)
    ...
    def bypass_by_nest_dim(self, x: torch.Tensor) -> bool:
        if self.nest_dim is None or not self.training:
            return False
        if x.dim() != 4:
            return False
        size = x.size(2) * x.size(3)
        return size <= self.nest_dim
